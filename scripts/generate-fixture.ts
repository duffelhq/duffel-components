...
const main = async () => {
  try {
    ...
    const description = `// This fixture was generated by scripts/generate-fixture.ts
// It includes an offer with ${withPlural(
      adultCount,
      "passenger",
      "passengers"
    )} and ${withPlural(sliceCount, "slice", "slices")}: ${sliceInput.join(
      " ⇢ "
    )}\n\n`;
    fs.writeFileSync(
      `src/fixtures/offers/${firstOffer.id}.js`,
      description +
        `export const offer = ` +
        JSON.stringify(firstOffer, null, 2)
    );
    ...
    fs.writeFileSync(
      `src/fixtures/seat-maps/${firstOffer.id}.js`,
      `export const seatMaps = ` + JSON.stringify(seatMaps, null, 2)
    );

    console.log(`\n🐄 Fixtures saved for ${firstOffer.id}`);
    console.log(`  ↳ /src/fixtures/offer/${firstOffer.id}.js`);
    console.log(`  ↳ /src/fixtures/seat-maps/${firstOffer.id}.js\n`);
  } catch (err) {
    console.error(err);
    process.exit(1);
  }
